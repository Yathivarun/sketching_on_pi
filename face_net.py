...
UPDATED: Improved normalization (0-1 scale, then [-1,1] range) and explicit embedding norm check
"""
...
def preprocess_recognition(face_img):
    ...
    UPDATED: Using improved normalization (0-1 scale, then [-1,1] range)
    """
    if face_img.size == 0:
        return None
    ...
    face = face.astype(np.float32) / 255.0  # Scale to [0, 1]
...
def generate_embedding(rec_sess, rec_input_name, face_img):
    ...
    UPDATED: More explicit normalization (flatten first, check norm, then normalize)
    """
    try:
        ...
        # Flatten and normalize (more explicit approach)
